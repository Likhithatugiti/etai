"""
Compliance Agent — regulatory gap detection, OISD/PESO/Factory Act mapping.
"""
...
def compliance_agent(state: AgentState) -> AgentState:
    if "compliance" not in state.get("intent_tags", []):
    ...
    for sq in state.get("sub_questions", [state["query"]]):
        prompt = COMPLIANCE_PROMPT.format(sub_question=sq, context=context)
